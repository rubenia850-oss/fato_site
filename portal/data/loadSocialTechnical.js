// data/loadSocialTechnical.js — carga das camadas social/technical (Sprint 6, M-02/M-03).
...
export function loadTagsAndFormats(db) {
  const tagColors = {};
  query(db, `SELECT t.name AS name, tm.color AS color FROM tags t JOIN tag_meta tm ON tm.tag_id=t.id`)
    .forEach(r => { tagColors[r.name] = r.color; });

  const formatMeta = {};
  query(db, `SELECT code, label, color, bg FROM format_meta`)
    .forEach(r => { formatMeta[r.code] = { label: r.label, color: r.color, bg: r.bg }; });

  return { tagColors, formatMeta };
}

// Tags por source_id (compartilhado entre M-02 e M-03)
export function loadSourceTags(db) {
  const rows = query(db, `SELECT st.source_id AS source_id, t.name AS name FROM source_tags st JOIN tags t ON t.id=st.tag_id`);
...
export function loadSocial(db, tagsBySource) {
  const rows = query(db, `
  ...
  // SP-55 (item 9 do estudo de viabilidade, decisão de produto confirmada 17/07): loadSocial() nunca
  // lia `source_cnct_profiles` — 699 vínculos fonte↔perfil existentes pra layer=social, 0 expostos
  // até agora (achado ao investigar o item 9, além do que o estudo original descrevia). Mesmo padrão
  // já usado em loadTechnical(). `guia_source_profiles` (a 2ª tabela do item 9) tem 0 linhas pra
  // layer=social — confirmado por query — então a "soma" pedida não muda nada aqui especificamente,
  // mas o achado de loadSocial nunca ter lido nem a 1ª tabela é o ganho real desta correção.
  const cnctRows = query(db, `
  ...
  // empresa que `loadSocial` nunca lia — a aba Social mostrava menos programas do que existem no
  // banco. Mesclado na mesma lista (não em seção separada), com id prefixado `sp-N` (mesmo padrão
  // da view `v_all_social_sources`, não usada diretamente aqui pois ela não traz todos os campos
  // que `SourceCard` espera — mapeado campo a campo abaixo). `sector_programs` não tem `url`: SP-56
  // também tornou o link "Acessar portal" do `SourceCard` condicional (antes renderizava sempre,
  // mesmo sem link — bug latente que só ficou visível ao introduzir uma fonte sem URL).
  const programRows = query(db, `
...
export function loadCourseEnrichment(db) {
  const courses = query(db, `SELECT id AS course_id, profile_id, nome FROM cnct_courses WHERE profile_id IS NOT NULL`);
  const courseByProfile = {};
  courses.forEach(c => { courseByProfile[c.profile_id] = c; });

  const cboRows = query(db, `SELECT course_id, codigo, descricao, principal FROM course_cbos`);
  const cboByCourse = groupBy(cboRows, "course_id");
  const certRows = query(db, `SELECT course_id, nome, principal FROM course_certificacoes`);
  const certByCourse = groupBy(certRows, "course_id");
  const normaRows = query(db, `SELECT course_id, norma FROM course_normas`);
...
export function loadTechnical(db, tagsBySource, courseEnrichment) {
  const rows = query(db, `
    SELECT s.id AS id, s.layer AS layer, c.name AS company, sec.name AS sector, s.uf AS uf,
           s.program AS program, s.format AS format, s.national AS national, s.free AS free,
           s.lang AS lang, s.cadastro AS cadastro, s.highlight AS highlight, s.audience AS audience,
  ...
  // SP-55 (item 9, decisão de produto confirmada 17/07): união pedida no estudo
  // (source_cnct_profiles + guia_source_profiles). Verificado contra o `.db` real: pra
  // layer=technical especificamente, `guia_source_profiles` contribui 0 pares novos (a tabela não
  // tem nenhuma linha pra essa camada — só 'guia'/'sector' têm). Aplicada mesmo assim, por
  // completude e caso o BANCO popule mais linhas no futuro; o ganho real desta sessão pro item 9
  // foi achar e corrigir `loadSocial()` (nunca lia nem a 1ª tabela, 699 vínculos expostos agora).
  const cnctRowsRaw = query(db, `
    SELECT source_id, profile_id FROM source_cnct_profiles
    UNION
    SELECT source_id, profile_id FROM guia_source_profiles
  `);
  const profileMeta = {};
  query(db, `SELECT id, code, name, tier FROM cnct_profiles`).forEach(p => { profileMeta[p.id] = p; });
  const cnctRows = cnctRowsRaw
    .map(r => ({ source_id: r.source_id, profile_id: r.profile_id, ...profileMeta[r.profile_id] }))
    .filter(r => r.code);
  const cnctBySource = groupBy(cnctRows, "source_id");

  // SP-48 (15/07): atlas_code sozinho não desambigua (20 dos códigos citados aqui se repetem
  // entre atlas diferentes). source_atlas_trails já tem atlas_num resolvido pra 194/232 (83%)
  // linhas (resolucao_metodo/resolucao_confianca) — os 38 restantes ficam sem atlas_num (null),
  // pedido formal registrado em _BACKLOG.md (SP-48) para a sessão BANCO completar a resolução.
  const atlasRows = query(db, `SELECT source_id AS source_id, atlas_code AS atlas_code, atlas_num AS atlas_num FROM source_atlas_trails`);
  const atlasBySource = groupBy(atlasRows, "source_id");

  // SP-54 (item 10 do estudo de viabilidade, decisão de produto confirmada 17/07): material_types
  // (16 tipos) via source_material_types. Cobertura real pra layer=technical: 101/572 (17,7%) —
  // maior que a estimativa de 6,5% do estudo original (que era sobre TODAS as camadas de `sources`,
  // 132/2.045), mas ainda baixa — badge aparece em ~1 a cada 6 fontes, não em todas.
  const matTypeMeta = {};
  query(db, `SELECT code, label_pt, description FROM material_types`).forEach(m => { matTypeMeta[m.code] = m; });
  const matBySource = {};
  query(db, `SELECT source_id, type_code, confidence FROM source_material_types`).forEach(r => {
  ...
  return rows.map(r => {
    ...
    return {
      ...
      caminhos_cbo: caminhosCbo, // EXP-02: {course_name, cbos:[...], certificacoes:[...], normas:[...]} | null
    };
  });
}
