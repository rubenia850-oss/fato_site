...
| SP-75 | **Achado ao revisar o handoff da migração pra API (24/09/2026, sessão SITE, antes de qualquer teste em navegador real — ver limite abaixo):** `index.html` ainda carregava `db.js` como módulo ES antes de montar o React ("Passo 1" do bootstrap antigo), com `mountApp()` bloqueado em `window.__fato_db_ready`. Resíduo de antes da migração — `App.jsx` e toda a árvore de módulos que ele importa não referenciam mais `db.js`/sql.js desde o SP-73. Na prática não travava a montagem (o `import` de `db.js` não dispara nenhum fetch no carregamento do módulo, só expõe funções nunca chamadas), mas é código morto com comentários defasados (citava `fato_v237.db` via sql.js) que confundiria a próxima sessão. | Portal | 🟢 | ✅ **Corrigido (24/09/2026)** — bootstrap do `db.js` removido de `index.html` (o `<script type="module">` do "Passo 1", a checagem `__fato_db_ready` em `mountApp()`, e os 2 casos especiais de `./db.js` no loader de módulos/shim `require`); comentário de topo de `App.jsx` atualizado pra descrever a arquitetura via API em vez de sql.js. `db.js` continua existindo no repo como legado morto, só não é mais carregado por `index.html` (mesma decisão de manter já registrada no `HANDOFF.md`). **Limite honesto: não testado num navegador real** — o ambiente desta sessão não tem acesso de rede pra abrir o portal contra a API viva (`fato-portal-api.rubenia850.workers.dev`); só checagem estática (sintaxe de cada bloco `<script>` validada com `node --check`, grep confirmando 0 referências residuais a `db.js`/`__fato_db_ready`/`fato:db-module-ready`). Item 1 do `HANDOFF.md` ("testar o portal localmente") continua pendente de verificação visual de fato. |
